...
// DOM Elements
const totalAppsEl = document.getElementById('totalApps') as HTMLElement;
const submittedAppsEl = document.getElementById('submittedApps') as HTMLElement;
const reviewAppsEl = document.getElementById('reviewApps') as HTMLElement;
const acceptedAppsEl = document.getElementById('acceptedApps') as HTMLElement;
const applicationsList = document.getElementById('applicationsList') as HTMLElement;
...
function updateStats() {
    ...
    const underReview = allApplications.filter(app => 
...
function handleSubmitApplication(e: Event) {
    if (!currentUser) return;
...
function handleEditNotes(e: Event) {
    if (!currentUser) return;
...
function handleDeleteApplication(e: Event) {
    const target = e.target as HTMLElement;
    const appId = target.dataset.appId;
...
function handleUpdateStatus(e: Event) {
    if (!currentUser) return;
    ...
    const statusProgression: { [key: string]: ApplicationStatus } = {
...
function handleViewProgram(e: Event) {
    const target = e.target as HTMLElement;
